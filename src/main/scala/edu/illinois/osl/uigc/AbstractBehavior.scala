...
abstract class AbstractBehavior[T](context: ActorContext[T])
  ...
  override final def receive(ctx: TypedActorContext[GCMessage[T]], msg: GCMessage[T]): Behavior[T] = {
    val appMsg = context.engine.onMessage(msg, context.state, context.rawContext)
    ...
    context.engine.onIdle(msg, context.state, context.rawContext) match {
      case _: Engine.ShouldStop.type => scaladsl.Behaviors.stopped
      case _: Engine.ShouldContinue.type => result
    }
  }

  override final def receiveSignal(ctx: TypedActorContext[GCMessage[T]], msg: Signal): Behavior[T] = {
    context.engine.preSignal(msg, context.state, context.rawContext)

    val result =
      onSignal.applyOrElse(msg, { case _ => scaladsl.Behaviors.unhandled }: PartialFunction[Signal, Behavior[T]]) 

    context.engine.postSignal(msg, context.state, context.rawContext) match {
